...
```  
If we know the values that shall go in the array before it is created so is it also possible to use a shorthand notation. That will allocate memory and create the array reference.
```Java
